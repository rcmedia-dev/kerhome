...
export const CompactPacoteCard: React.FC<CompactPacoteCardProps> = ({
  ...
  <div
    className={`relative rounded-2xl p-6 cursor-pointer transition-all duration-300 border-2 ${isSelected
  ...
  >
    ...
    <div className="text-center mb-4">
      <div className={`inline-flex items-center justify-center w-12 h-12 rounded-2xl mb-3 ${pacote.corDestaque === 'purple'
    ...
    <div className="space-y-2 mb-4">
      {pacote.beneficios.slice(0, 3).map((beneficio, index) => (
        <div key={index} className="flex items-center gap-2 text-xs text-gray-700">
          <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${pacote.corDestaque === 'purple' ? 'bg-purple-100' : 'bg-orange-100'
            }`}>
    ...
    <button className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${isSelected
